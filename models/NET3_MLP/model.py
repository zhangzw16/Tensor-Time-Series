...
class MLPModel(nn.Module):
    def __init__(self, n1, n2, dim_in = 1, dim_out = 8):
        super(MLPModel, self).__init__()
        self.n1 = n1
        self.n2 = n2
        # 定义MLP层
        self.fc1 = nn.Linear(n1*n2*dim_in, 128)  # 第一个全连接层
        self.fc2 = nn.Linear(128, n1*n2*dim_out)  # 第二个全连接层，输出维度为n1*n2*8
...
class NET3(nn.Module):
    def __init__(self, configs=None):
        ...
        MLPconfig = self.configs["TGCN"]
        n1 = self.configs["mode_dims"][0]
        n2 = self.configs["mode_dims"][1]
        self.mlp = MLPModel(n1, n2, MLPconfig["dim_input"], MLPconfig["dim_output"])
...
Tensor Model: NET3
'''
class NET3_MLP_TensorModel(TensorModelBase):
