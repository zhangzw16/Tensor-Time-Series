...
class DCGRUDecoder(nn.Module):
    ...
    def forward(self, inputs, initial_hidden_state, teacher_forcing_ratio=0.5):
        ...
        outputs = torch.zeros(seq_length, batch_size, self._num_nodes*self._output_dim).cuda()  # (13, 50, 207*1)
...
class DCRNNModel(nn.Module):
    def __init__(self, adj_mat, enc_input_dim, dec_input_dim, max_diffusion_step, num_nodes,
                 num_rnn_layers, rnn_units, seq_len, pred_len, output_dim, filter_type):
        ...
        # self.GO_Symbol = torch.zeros(1, batch_size, num_nodes ,self._output_dim).cuda()
    ...
    def forward(self, source, target, teacher_forcing_ratio):
        ...
        dim0, dim1, dim2, dim3 = target.size()
        GO_Symbol = torch.zeros(1, dim1, dim2, dim3).cuda()
        target = torch.cat([GO_Symbol, target], dim=0)
        
        # initialize the hidden state of the encoder
        init_hidden_state = self.encoder.init_hidden(dim1).cuda()
        ...
        outputs = outputs[1:, :, :].view(self._pred_len, dim1, self._num_nodes, self._output_dim)
        return outputs
