...
if __name__ == '__main__':
    ...
    task_name = 'Task5-GraphModelAblations-GWNs'
    task_name = 'Task5-GraphModelAblations-MTGNNs'
    task_name = 'Task5-GraphModelAblations-GCGRUs'
    pred_len = [12, 48, 96]
    ...
    data_mode = 0
    pass
    # model_list = ['GraphWaveNet_unit','GraphWaveNet_wo_GCN','GraphWaveNet_wo_TCN','GraphWaveNet_wo_GCN_TCN']
    # model_list = ['MTGNN_unit','MTGNN_wo_GCN']
    # model_list = ['GCGRU_unit','GCGRU_wo_GCN','GCGRU_wo_GRU','GCGRU_wo_GCN_GRU']
    ...
    for model in model_list:
        for dataset_name in dataset_list:
            for pred in pred_len:
                input_len = best_input_reader.get_best_input(dataset_name, model.split('_')[0], pred)
                script_helper.add_task([dataset_name], [model], data_mode, [input_len], [pred], batch_size)
    script_helper.save_to_file(f'run_MTS-[GWNs]-mode-{data_mode}.sh')
    # -------------------------------
    ...
    for model in model_list:
        for dataset_name in dataset_list:
            for pred in pred_len:
                input_len = best_input_reader.get_best_input(dataset_name, model.split('_')[0], pred)
                script_helper.add_task([dataset_name], [model], data_mode, [input_len], [pred], batch_size)
    script_helper.save_to_file(f'run_MTS-[MTGNNs]-mode-{data_mode}.sh')
    # -------------------------------

    task_name = 'Task5-GraphModelAblations-GCGRUs'
    model_list = ['GCGRU','GCGRU_wo_GCN','GCGRU_wo_GRU','GCGRU_wo_GCN_GRU']
    script_helper = SctriptHelper(task_name)
    data_mode = 0
    batch_size = 0
    for model in model_list:
        for dataset_name in dataset_list:
            for pred in pred_len:
                input_len = best_input_reader.get_best_input(dataset_name, model.split('_')[0], pred)
                script_helper.add_task([dataset_name], [model], data_mode, [input_len], [pred], batch_size)
    script_helper.save_to_file(f'run_MTS-[GCGRUs]-mode-{data_mode}.sh')
