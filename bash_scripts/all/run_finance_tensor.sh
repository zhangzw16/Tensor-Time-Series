...
for his_len in ${HisLenList[@]}; do
    for pred_len in ${PredLenList[@]}; do
        ...
        for model in ${TensorModelList[@]}; do
            ...
            bash bash_scripts/run_finance.sh --model $model --his_len $his_len --pred_len $pred_len --output_dir $output_dir --task_name $task_name --dataset_base $dataset_base --seed $seed
        done
        # -------------------------------------------
        # Multivar Model
